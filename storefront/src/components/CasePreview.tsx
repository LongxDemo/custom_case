...
/* ───────────────────────── Realistic camera module (CSS) ─────────────────────────
   Mirrors admin/src/components/CasePreview.tsx — same brand→layout mapping. */
...
export type CamStyle = 'ip17-plateau' | 'ip17-bar' | 'ip17-air' | 'ip-square' | 'ip-vert' | 'ip-dual' | 'ip-single' | 'samsung' | 'pixel' | 'generic';
...
export function CameraModule({ style, width: W, height: H, tint }: { style: CamStyle; width: number; height: number; tint: string }) {
  ...
  if (style === 'ip17-air') {
    ...
    // Bar height per spec table (~20mm on a 74.7mm-wide body, ~27%) — was 0.17.
  ...
  if (style === 'ip-single') {
    // Spec table: ~24mm round housing on a ~67-76mm-wide body, ~0.32 -
    // was 0.13, less than half size.
    const ld = W * 0.32;
