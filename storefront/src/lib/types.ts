...
export type PhoneModel = { id: string; brand: string; name: string; heightMm: number; widthMm: number };
...
export const REFERENCE_WIDTH_MM = Math.max(...phoneModels.map((m) => m.widthMm));
